...
logger = logging.getLogger("adversarial.pathogen_agent")

class PathogenPolicyNetwork(nn.Module):
    """
    Policy Network for the Artificial Pathogen.
    Outputs action type (discrete), target component (discrete), and magnitude (continuous delta).
    """
    def __init__(self, state_dim: int, num_action_types: int = 5, num_targets: int = 46):
        super(PathogenPolicyNetwork, self).__init__()
        ...
        # log_std head for standard deviation parameterization
...
class PathogenAgent:
    """
    Pathogen PPO Agent implementing PPO-Clip updates for hybrid discrete-continuous actions.
    """
    def __init__(self, state_dim: int = 293, lr_actor: float = 3e-4, lr_critic: float = 1e-3, device: str = "cpu"):
        self.device = torch.device(device)
        
        self.actor = PathogenPolicyNetwork(state_dim).to(self.device)
        self.critic = PathogenValueNetwork(state_dim).to(self.device)
    ...
    def select_action(self, state: np.ndarray, evaluation: bool = False) -> Tuple[Dict[str, Any], float, float]:
        """
        Samples a hybrid action vector.
        Returns:
            action: Dict containing type, target, and magnitude
            log_prob: float (total sum of action component log probs)
            value: float
        """
        state_t = torch.FloatTensor(state).to(self.device).unsqueeze(0)
        
        with torch.no_grad():
            act_logits, trg_logits, mag_mean, mag_std = self.actor(state_t)
        ...
        if evaluation:
            ...
            # Combined joint log probability of the action vector components
            log_prob = (
                dist_act.log_prob(act_type_t).item() +
                dist_trg.log_prob(act_target_t).item() +
                dist_mag.log_prob(act_mag_t).sum().item()
            )
            
        action = {
            "type": act_type,
            "target": act_target,
            "magnitude": np.array([np.clip(act_mag, -0.20, 0.20)], dtype=np.float32)
        }
        
        return action, log_prob, value

    def update(self, 
               memory_data: Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray],
        ...
        """
        states, act_types, act_targets, act_mags, old_log_probs, values, rewards, dones = memory_data
        ...
        act_mags_t = torch.FloatTensor(act_mags).to(self.device)
        ...
        for _ in range(ppo_epochs):
            ...
            for start_idx in range(0, dataset_size, batch_size):
                ...
                act_logits, trg_logits, mag_mean, mag_std = self.actor(b_states)
                ...
                new_log_probs = (
                    ...
                    dist_mag.log_prob(b_mags).squeeze(-1)
                )
    ...
    def save_checkpoint(self, directory: str, filename: str = "ppo_pathogen.pt"):
