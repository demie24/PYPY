...
CHECKPOINT_PATH = os.path.join(MODEL_DIR, "lstm_multi_bus_predictor.pt")
...
def run_training():
    ...
    dataset = TelemetryDataset(CSV_PATH, window_size=window_size, target_index=target_indices)
    ...
    print(f"LSTM dimensions: Sequence Length = {window_size}, Input Feature Count = {feature_dim}, Output Count = 5")
    model = MultiBusPredictorLSTM(input_dim=feature_dim, output_dim=5, hidden_dim=64, num_layers=2, dropout=0.2)
    ...
    # 6. Training loop
    epochs = 10
    print(f"Starting LSTM multi-bus voltage predictor training loop ({epochs} epochs)...")
    ...
    model.eval()
    test_seq, test_targets = val_dataset[0]
    test_input = test_seq.unsqueeze(0) # add batch dim
    
    with torch.no_grad():
        pred_vals = model(test_input).squeeze(0).numpy()
        
    actual_voltages = test_targets.numpy()
    
    print(f"\n--- Multi-Bus Voltage Validation Inference ---")
    print(f"Input Shape: {test_input.shape}")
    buses_labels = ["Bus_1", "Bus_3", "Bus_5", "Bus_7", "Bus_9"]
    for idx, label in enumerate(buses_labels):
        act_v = round(float(actual_voltages[idx]), 4)
        pred_v = round(float(pred_vals[idx]), 4)
        print(f" {label} Voltage -> Actual: {act_v} p.u. | Predicted: {pred_v} p.u. (deviation: {abs(act_v - pred_v):.4f})")
        assert not np.isnan(pred_vals[idx]), f"Prediction for {label} returned NaN value!"
