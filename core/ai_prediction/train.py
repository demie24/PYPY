...
CHECKPOINT_PATH = os.path.join(MODEL_DIR, "lstm_threat_predictor.pt")
...
def generate_synthetic_dataset(num_samples=100):
    ...
    representing normal states, cyber attack escalations, and FLISR restoration sequences.
    """
    ...
    for i in range(num_samples):
        ...
        # Cyber attack simulation phase: samples 50 to 80
        if 50 <= i < 80:
            attack_active = 1
            # Voltage collapse on Bus 5
            voltages[4] = round(voltages[4] - (i - 50) * 0.012, 4)
            # Load overload on L4_5
            loads[3] = round(loads[3] + (i - 50) * 3.5, 2)
            anomaly = round(0.005 + (i - 50) * 0.0008, 6)
            threat = int(min(100, (i - 50) * 3.3))
            
            # Breaker trips near step 70
            if i >= 70:
                breakers[3] = 0  # L4_5 breaker opens
                flisr_state = 1  # FAULT_DETECTED
                
        # Self-healing and FLISR restoration phase: samples 80 to 100
        elif 80 <= i:
            breakers[3] = 0
            # Tie breaker L7_8 closes
            breakers[7] = 1
            flisr_state = 4  # RESTORED state
...
def run_training():
    # 1. Fallback dataset collection validation
    ...
    # 2. Instantiate loader and pre-process
    window_size = 10
    dataset = TelemetryDataset(CSV_PATH, window_size=window_size)
    ...
    # 4. Instantiate model
    # Get feature size from first dataset sequence sample (shape: seq_len, feature_dim)
    ...
    model = ThreatPredictorLSTM(input_dim=feature_dim, hidden_dim=64, num_layers=2, dropout=0.2)
    ...
    # 6. Training loop
    epochs = 10
    print(f"Starting LSTM threat predictor training loop ({epochs} epochs)...")
    ...
    # Add batch dimension: shape becomes (1, window_size, feature_dim)
    test_input = test_seq.unsqueeze(0)
    
    with torch.no_grad():
        pred_scaled = model(test_input).item()
        
    actual_threat = round(float(test_target.item()) * 100, 1)
    predicted_threat = round(pred_scaled * 100, 1)
    
    print(f"\n--- Validation Inference Test ---")
    print(f"Input Shape: {test_input.shape}")
    print(f"Actual Threat Score Target: {actual_threat}")
    print(f"Predicted Threat Score: {predicted_threat} (deviation: {abs(actual_threat - predicted_threat):.1f})")
    
    # Assert check to ensure no NaNs are returned
    assert not np.isnan(pred_scaled), "Prediction returned NaN value!"
    print("Inference test complete. No NaNs detected. Validation successful!")
