...
class RecoveryRewardEngine:
    ...
    def evaluate_restoration_quality(self, 
        ...
        recovery_completed = prev_islanded and not curr_islanded and np.all(curr_voltages > 0.90) and np.all(curr_voltages < 1.10)
        ...
        details["penalty_rollback_restoration"] = -30.0 if rollback_occurred else 0.0

        # 4. Faster restoration time (penalize steps taken exceeding 3)
        details["penalty_restoration_speed"] = float(-2.0 * max(0, step_count - 3))

        # 4A. Restoration Efficiency (additional reward for finishing in fewer switching steps)
        details["reward_restoration_efficiency"] = float(max(0.0, 20.0 - 4.0 * step_count)) if recovery_completed else 0.0
